...
# To download notebooks and datafiles, as well as get help on Jupyter
# notebooks in the Coursera platform, visit the [Jupyter Notebook FAQ]
...
# various supervised learning models. Part 1 of this assignment will look at 
...
# `X_train` for degrees 1, 3, 6, and 9. 
...
# The first row of this array should correspond to the output from 
# the model trained on degree 1, the second row degree 3, the third 
# row degree 6, and the fourth row degree 9.
# 
...
# the training data `X_train` for degrees 0 through 9. For each model 
# compute the $R^2$ (coefficient of determination) regression score on 
# the training data as well as the the test data, and return both of 
# these arrays in a tuple.
# 
...
# (with parameters `alpha=0.01`, `max_iter=10000`) both on polynomial 
# features of degree 12. Return the $R^2$ score for both the 
# LinearRegression and Lasso model's test sets.
# 
...
# stored in `readonly/mushrooms.csv`. The data will be used to train a model to predict whether
...
# Recall that the validation_curve function, in addition to taking an 
# initialized unfitted classifier object, takes a dataset as input and 
# does its own internal train-test splits to compute results.
# 
...
# the validation curve function (instead of X_mush and y_mush) to reduce computation time.**
# 
...
# 
# With this classifier, and the dataset in X_subset, y_subset, explore the effect of `gamma`
# on classifier accuracy by using the `validation_curve` function to find the training and test 
# scores for 6 values of `gamma` from `0.0001` to `10` (i.e. `np.logspace(-4,1,6)`). Recall that 
# you can specify what scoring metric you want validation_curve to use by setting the "scoring"
# parameter.  In this case, we want to use "accuracy" as the scoring metric.
# 
# For each level of `gamma`, `validation_curve` will fit 3 models on different subsets of the 
# data, returning two 6x3 (6 levels of gamma x 3 fits per level) arrays of the scores for the 
...
# ### Question 7
# 
# Based on the scores from question 6, what gamma value corresponds to a model that is
# underfitting (and has the worst test set accuracy)? What gamma value corresponds to a model 
# that is overfitting (and has the worst test set accuracy)? What choice of gamma would be the 
# best choice for a model with good generalization performance on this dataset (high accuracy 
